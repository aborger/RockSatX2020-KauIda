...
class Rf(Device):
	...
	def __activate_thread(self):
		...
		file.close()
		write_telem(self.sensor_values)
...
# dutyCycles: 0 degrees = 2.5, 90 degrees = 7.5, 180 degrees = 12.5
class Lock(Device):
	def __init__(self, pin):
		super().__init__('Servo Lock')
		GPIO.setup(pin, GPIO.OUT)
		self.servo = GPIO.PWM(pin, 50) # 50 is the PWM frequency
...
#			Led					#
#---------------------------------------------------------------#
class ReadyLight(Device):
	def __init__(self, pin):
		super().__init__('Ready Light')
		GPIO.setup(pin, GPIO.OUT)
		self.pin = pin
